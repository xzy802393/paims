...
import com.cz.yingpu.frame.dao.dialect.IDialect;
import com.cz.yingpu.frame.entity.AuditLog;
import com.cz.yingpu.frame.task.LuceneTask;
...
import com.cz.yingpu.frame.util.RegexValidateUtils;
import com.cz.yingpu.frame.util.SecUtils;
import com.cz.yingpu.frame.util.ThreadPoolManager;
import com.cz.yingpu.frame.util.WhereSQLInfo;

/**
 * 基础的Dao父类,所有的Dao都必须继承此类,每个数据库都需要一个实现.</br>
 *
 * 例如 demo数据的实现类是com.cz.yingpu.springrain.dao.BasedemoDaoImpl,demo2数据的实现类是org.
 * springrain demo2.dao.Basedemo2DaoImpl</br>
 *
 * @copyright {@link 9iu.org}
 * @author springrain<Auto generate>
 * @version 2013-03-19 11:08:15
 * @see com.cz.yingpu.frame.dao.BaseJdbcDaoImpl
 */
public abstract class BaseJdbcDaoImpl extends BaseLogger implements IBaseJdbcDao {
	 ...
	 * 例如:demo数据库的代理Dao com.cz.yingpu.springrain.dao.BasedemoDaoImpll
	 * 实现返回的是spring的beanjdbc.</br>
	 * demo2 数据库的代理Dao com.cz.yingpu.demo2.dao.Basedemo2DaoImpl
	 ...
	 * 例如:demo 数据库的代理Dao com.cz.yingpu.springrain.dao.BasedemoDaoImpl
	 * 实现返回的是spring的bean jdbcCall.</br>
	 * datalog 数据库的代理Dao com.cz.yingpu.demo2.dao.Basedemo2DaoImpl
	 ...
	 * 在entity使用@PKSequence实现自增 详见 com.cz.yingpu.frame.dao.dialect.IDialect的实现
	 *
	 * @return
	 */
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private String warpsavesql(Object entity, Map paramMap, Boolean isSequence, boolean ignoreNull) throws Exception {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private Object saveNoLog(Object entity, boolean ignoreNull) throws Exception {
		// entity信息
		EntityInfo entityInfo = ClassUtils.getEntityInfoByEntity(entity);
		Class<?> returnType = entityInfo.getPkReturnType();
		...
		String sql = warpsavesql(entity, paramMap, isSequence, ignoreNull);
		// 打印sql
		logInfoSql(sql);
	...
	public Object save(Object entity, boolean ignoreNull) throws Exception {
		// 保存到数据库
		Object id = saveNoLog(entity, ignoreNull);
	...
	public List<Integer> save(List list) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			Map paramMap = new HashMap();
			sql = warpsavesql(list.get(i), paramMap, false, false);
			maps[i] = paramMap;
		}
		int[] batchUpdate = getWriteJdbc().batchUpdate(sql, SqlParameterSourceUtils.createBatch(maps));
	...
	@Override
	public Integer updateValidValue(Object entity) throws Exception {
	...
	public Map<String, Object> queryObjectByProc(Finder finder) throws Exception {
		...
		JdbcTemplate jt = getJdbcCall().getJdbcTemplate();
		if (StringUtils.isNotBlank(procName)) {
			SimpleJdbcCall call=new SimpleJdbcCall(getJdbcCall().getJdbcTemplate());
			simpleJdbcCall = call.withProcedureName(procName);
		} else {
			simpleJdbcCall = getJdbcCall().withFunctionName(functionName);
		}
