...
public class SecurityUtils {
	...
	public static void initPrivateKey(){
		try {
				if(privateKey==null){
					privateKeyPath = com.fuiou.util.ConfigReader.getConfig("privateKeyPath");
	...
	public static void initPublicKey(){
		try {
			if(publicKey==null){
				publicKeyPath = com.fuiou.util.ConfigReader.getConfig("publicKeyPath");
	...
	public static String sign(String inputStr) {
		String result = null;
		  try {
	            ...
	            byte[] tByte;
	            System.out.println("签名明文"+inputStr);
	            Signature signature = Signature.getInstance("SHA1withRSA","BC");
	            signature.initSign(privateKey);
	            signature.update(inputStr.getBytes("UTF-8"));
	            tByte = signature.sign();
	            result = com.fuiou.util.Base64.encode(tByte);
	            System.out.println("签名密文"+result);
	        }
	        catch (Exception e) {
	        	e.printStackTrace();
	        	System.out.println("密钥初始化失败");
	...
	public static boolean verifySign(String src,String signValue) {
		  ...
		  try {
	            ...
	            bool = signature.verify(com.fuiou.util.Base64.decode(signValue));
	        }
	        catch (Exception e) {
	        	e.printStackTrace();
	        	System.out.println("密钥初始化失败");
	...
	private static PrivateKey getPrivateKey(String filePath) {
		...
		try {
			...
			PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(com.fuiou.util.Base64.decode(base64edKey));
