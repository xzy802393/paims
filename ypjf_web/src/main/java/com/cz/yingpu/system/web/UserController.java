...
public class UserController extends BaseController {
	...
	public @ResponseBody
	Map<String, Object> checkPwd(HttpServletRequest request, HttpServletResponse response, Model model)throws Exception{
		String userId = SessionUser.getUserId();
	...
	public @ResponseBody ReturnDatas selectUser(HttpServletRequest request){
		ReturnDatas returnDatas = ReturnDatas.getSuccessReturnDatas();
		try {
			List<Map<String,Object>> users = userService.queryForList(new Finder("SELECT * FROM t_user WHERE account=:account limit 1").setParam("account",request.getParameter("account")));

			if (users.size()>0){
				returnDatas.setMessage("账号已存在");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
