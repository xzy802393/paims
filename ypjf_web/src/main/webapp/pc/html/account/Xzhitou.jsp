...
<!DOCTYPE html>
	...
	<iframe src="/yingpu/pc/head3.html?index=6" height="104px" width="100%" scrolling="no" name="head" style="border:0px;"></iframe>
					...
					<div id="toubiao" style="display: block;">
						<table border="0">
							<tr class="title">
								...
								<th>投资日期</th>
								<th>期限(月)</th>
								<th id="expiresDate" style="display: none;">到期时间</th>
								<th>本金(元)</th>
								<th>利息(元)</th>
								<th>优惠券(元)</th>
								<th>本息合计(元)</th>
		...
		<script src="/yingpu/pc/my-invest/js/utils.js" type="text/javascript" charset="utf-8"></script>
		...
		<script src="../account/js/zhitou.js" type="text/javascript" charset="utf-8"></script>
	...
	<script>
        ...
        loadList();
        function loadList() {
            $.post('/yingpu/pc/myaccount/getSmartOrder', {
                pageIndex: pageIndex,
                id: '${pcuser.id}',
                ...
                if (data==undefined || data.length == 0 ){
                    ...
                    $.each(data, function (i, o) {
                        html += '<tr style="cursor: pointer;" onclick="reinvestPage(' + o.oid + ')">';
                        html += '<td title="'+ o.name +'">' + o.name + '</td>';
                        html += '<td>' + (o.rate * 100).toFixed(2) + '%</td>';
                        html += '<td>' + new Date(o.createtime).Format('yyyy-MM-dd') + '</td>';
                        html += '<td>' + o.deadline + '个月</td>';
                        if($("#expiresDate").css('display')=='none'){

						}else{
                            html += '<td class="daoqi">' + new Date(o.expiresdate).Format('yyyy-MM-dd') + '</td>';
						}
                        html += '<td>' + commafy(returnFloat(o.money)) + '</td>';
                        html += '<td>' + commafy(returnFloat(o.money * o.rate /12 * o.deadline)) + '</td>';
//							html+='<td>'+new Date(o.createTime).Format('yyyy-MM-dd hh:mm:ss')+'</td>';
                        if (o.amount == null) {
                            html += '<td class="yuan5"><span>无</span></td>';
                        } else {
                            html += '<td class="yuan5"><span>' + commafy(returnFloat(o.amount)) + '</span></td>';
                        }
                        html += '<td>' + commafy(returnFloat(o.money+o.money * o.rate /12 * o.deadline)) + '</td>';

                        if(o.ostatus==1){
                            html +='<td onclick="viewContract(${pcuser.id},pid='+o.projectId+',id='+o.oid+')"><span style="cursor:pointer; color:coral;">查看合同</span></td>';
                        }
                        html +='</tr>';
        ...
        function viewContract(ud,pd,d) {
            ...
            layer.open({
                ...
                shade: 0.8,
                area: ['50%', '80%'],
                content: '<%=basePath%>app/userproject/getSmartHeTong/json?' + param
            });
        }
        function reinvestPage(id) {
            location.href = '/yingpu/pc/myaccount/smartinvest/detail?id=' + id;
        }
	</script>
	</body>
